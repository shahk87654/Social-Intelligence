...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
    try {
      ...
      const result = await pool.query(
        "UPDATE posts SET tags = $1, note = $2, assigned_to = $3 WHERE id = $4 RETURNING tags, note, assigned_to",
        [tags, note, Number.isInteger(assignedTo) ? assignedTo : null, Number(params.id)]
      );
      if (!result.rowCount) return NextResponse.json({ error: "Post not found." }, { status: 404 });
